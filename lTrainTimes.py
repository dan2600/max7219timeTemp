...
# a global list needs to be created to collect the various times
    ...
    station_time_lookup(realtime_data, 'L16N')
...
# Grab the current time so that you can find out the minutes to arrival
    ...
    if time_until_train > 7 or time_until_next <= time_until_train:
        ...
        return "L in "+str(time_until_next)+ " min"
    elif time_until_train <= 0 and time_until_next <= 0:
        return "L train delays"
    else:
        return "L in "+str(time_until_train) + " & " +str(time_until_next)+ " min"
